...
# zendDebugger -> http://livebookmark.net/journal/2008/02/20/is-it-firefox-or-zend-debugger-cookie-standards/
# 
matches [
# 2010-10-14
...
# X-Powered-By: ZendServer/4.0
# Set-Cookie: ZDEDebuggerPresent=php,phtml,php3; path=/

      m=[]
      version = ''	
      m << {:string=>"Zend Debugger Enabled" } if @meta["set-cookie"] =~ /ZDEDebuggerPresent=[A-Za-z0-9\.\,\;]+.*/
      ...
      unless @meta.nil?
		...
		if xpoweredby =~ /Zend Core\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i			
			if version != ''
				version += ', Zend Core: ' + xpoweredby.scan(/Zend Core\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			else
				version += 'Zend Core: ' + xpoweredby.scan(/Zend Core\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			end
		elsif server =~ /Zend Core\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i
			if version != ''
				version += ', Zend Core: ' + server.scan(/Zend Core\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			else
				version += 'Zend Core: ' + server.scan(/Zend Core\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			end
		end 

		if xpoweredby =~ /Zend.?Server\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i			
			if version != ''
				version += ', Zend Server: ' + xpoweredby.scan(/Zend.? Server\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			else
				version += 'Zend Server: ' + xpoweredby.scan(/Zend.?Server\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			end
		elsif server =~ /Zend.?Server\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i
			if version != ''
				version += ', Zend Server: ' + server.scan(/Zend.?Server\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			else
				version += 'Zend Server: ' + server.scan(/Zend.?Server\/([a-zA-Z0-9\.\/\+\-\(\)]+)/i)[0].to_s 
			end
		end 
